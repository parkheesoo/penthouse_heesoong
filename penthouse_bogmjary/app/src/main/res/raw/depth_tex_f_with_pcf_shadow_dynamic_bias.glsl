...
float calcBias()
{
	...
	bias = clamp(bias, 0.0, 0.001); //작을수록 바닥 경계면이 잘맞음.
...
{        
    ...
    else if (vColor.w == -1.0)
    {
        ambientComponent = 0.25;
        if (diffuseComponent <= 0.0) shadow = 1.0;
        gl_FragColor = (texture2D(uTexture, v_texCoord) * (diffuseComponent + ambientComponent) * shadow);
    }
    else if (vColor.w == -2.0)
    {
        ambientComponent = 0.15;
        //if (diffuseComponent > 0.0) ambientComponent = 0.05;
        gl_FragColor = (texture2D(uPlaneTexture, v_texCoord) * (diffuseComponent + ambientComponent) * shadow);
    }
    //gl_FragColor = texture2D(uShadowTexture, v_texCoord);
}                                                                     	
