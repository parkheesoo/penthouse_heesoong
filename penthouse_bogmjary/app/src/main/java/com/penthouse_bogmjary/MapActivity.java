...
public class MapActivity extends AppCompatActivity implements MapView.MapViewEventListener, MapView.POIItemEventListener, MapView.OpenAPIKeyAuthenticationResultListener, View.OnClickListener, MapView.CurrentLocationEventListener {
        ...
        switch (id) {
            case R.id.fab:
                FancyToast.makeText(this, "1번 버튼: 검색좌표 기준으로 1km 검색" +
                        "\n2번 버튼: 검색좌표 기준 반경 변경 및 카테고리 변경" + "\n3번 버튼: 현재 매물 좌표 기준 1km 검색" + "\n4번 버튼: 현재 매물 좌표 기준 반경 이내 검색", FancyToast.LENGTH_SHORT, FancyToast.INFO, true).show();
                anim();
                break;
            case R.id.fab1:
            ...
            case R.id.fab3:
                ...
                isTrackingMode = false;
                FancyToast.makeText(this, "현재위치기준 반경 검색 시작", FancyToast.LENGTH_SHORT, FancyToast.SUCCESS, true).show();
                stopTrackingFab.setVisibility(View.GONE);
                mLoaderLayout.setVisibility(View.VISIBLE);
                anim();
                //현재 위치 기준으로 1km 검색
                                                                                                            ...
                                                                                                            public void onResponse(@NotNull Call<CategoryResult> call, @NotNull Response<CategoryResult> response) {
                                                                                                                if (response.isSuccessful()) {
                                                                                                                    ...
                                                                                                                    for (Document document : foodList) {
                                                                                                                        ...
                                                                                                                        marker.setMapPoint(mapPoint);
                                                                                                                        marker.setMarkerType(MapPOIItem.MarkerType.CustomImage); // 마커타입을 커스텀 마커로 지정.
                                                                                                                        marker.setCustomImageResourceId(R.drawable.ic_food_marker); // 마커 이미지.
                                                                                                                        marker.setCustomImageAutoscale(false); // hdpi, xhdpi 등 안드로이드 플랫폼의 스케일을 사용할 경우 지도 라이브러리의 스케일 기능을 꺼줌.
                                                                                                                        marker.setCustomImageAnchor(0.5f, 1.0f);
                                                                                                                        mMapView.addPOIItem(marker);
                                                                                                                    }
    ...
    public void onCurrentLocationUpdate(MapView mapView, MapPoint mapPoint, float accuracyInMeters) {
        ...
        if (list != null)
        {
            mCurrentLat = list.get(0).getLatitude();        // 위도
            mCurrentLng = list.get(0).getLongitude();    // 경도
        }
